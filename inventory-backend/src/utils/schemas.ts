...
const invoiceItemSchema = z.object({
  ...
  preparedBy: z.string().trim().max(100).nullable().optional(),
...
export const updateSettingsSchema = z.object({
  body: z
    .object({
      ...
      catalogStudioEnabled: z.boolean().optional(),
      catalogStudioDefault: z.enum(["store", "studio"]).optional(),
      catalogStudioPerRow: z.coerce.number().int().min(1).max(5).optional(),
      catalogStudioShape: z.enum(["square", "natural"]).optional(),
      catalogStudioOfferAlbum: z.boolean().optional(),
      catalogStudioNewAlbum: z.boolean().optional(),
      catalogStudioOfferDot: z.boolean().optional(),
      ...
      catalogSections: nullAsUndefined(
        z.array(z.object({ key: z.string().trim().min(1).max(40), enabled: z.boolean() })),
      ),
      catalogTexts: nullAsUndefined(z.record(z.string(), z.string())),
      catalogHiddenCategories: nullAsUndefined(z.array(z.string().trim())),
      catalogCategoryOrder: nullAsUndefined(z.array(z.string().trim())),
      catalogFeaturedProductIds: nullAsUndefined(z.array(z.string().uuid())),
      catalogDefaultView: nullAsUndefined(z.enum(["grid", "list"])),
      catalogDefaultPerRow: nullAsUndefined(z.coerce.number().int().min(1).max(4)),
      catalogDefaultSort: nullAsUndefined(z.string().trim().max(40)),
