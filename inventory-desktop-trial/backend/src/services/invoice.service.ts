...
async function couponDiscount(
  ...
  // SQLite uses file-level locking — no row-level SELECT FOR UPDATE needed
  const foundCoupon = await tx.coupon.findUnique({ where: { code: normalizedCode } });
  if (!foundCoupon) throw new AppError("Coupon is not active", 400, "COUPON_INACTIVE");
...
async function lockCustomer(_tx: Db, _customerId: string) {
  // SQLite uses file-level locking — no row-level FOR UPDATE needed
}
...
async function createInvoiceInTransaction(
  ...
  // feeds the loyalty-points formula below.
  let invoiceProfit = 0;
  ...
  // brand-new ACTIVE sale invoices only — never recomputed on edit.
...
async function updateInvoiceInTransaction(
  ...
  updatedBy: string
    ...
    const result = await createInvoiceInTransaction(
      tx,
      { ...input, type: input.type ?? invoice.type, customerId: newCustomerId },
      updatedBy,
      id,
