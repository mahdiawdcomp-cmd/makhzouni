...
export async function adjustWarehouseStock(
  ...
  // SQLite: read stock directly (no row-level FOR UPDATE — file-level locking)
  const stockRow = await db.productWarehouseStock.findUnique({
    where: { productId_warehouseId: { productId: input.productId, warehouseId: input.warehouseId } },
    select: { quantityPieces: true },
  });
  const balanceBefore = stockRow?.quantityPieces ?? 0;
