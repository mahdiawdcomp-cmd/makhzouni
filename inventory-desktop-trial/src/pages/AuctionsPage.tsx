      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {active && (
          <>
            ...
            {/* The desktop webview cannot open a new window — hand links to the OS. */}
            <Button size="sm" variant="outline" onClick={() => void openExternalUrl(`https://wa.me/?text=${encodeURIComponent(shareText)}`)}>
              <MessageCircle className="h-4 w-4" /> مشاركة بالواتساب
            </Button>
            <Button size="sm" variant="outline" onClick={() => void openExternalUrl(url)}>
              فتح صفحة المزاد
            </Button>
          </>
        )}
