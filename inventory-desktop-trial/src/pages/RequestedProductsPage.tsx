          ...
          <CardContent className="space-y-2 p-3">
            <div className="flex flex-wrap items-start gap-3">
              ...
              <div className="flex shrink-0 gap-2">
                {/* In the desktop shell an external <a> opens nothing — the
                    link has to go through the OS browser. */}
                <Button
                  size="sm"
                  variant="outline"
                  className="text-emerald-700"
                  onClick={() => void openExternalUrl(`https://wa.me/${row.phone.replace(/\D/g, "")}`)}
                >
                  رد بالواتساب
                </Button>
                {status === "OPEN" ? (
